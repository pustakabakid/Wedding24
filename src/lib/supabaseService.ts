...
function mapDbRowToSettings(data: any): FullInvitationSettings {
  return {
    ...
    invitation_type: data.invitation_type || (data.id === 'groom' ? 'groom' : 'bride'),
    theme_id: data.theme_id || 'classic-card',
...
export async function saveInvitationSettings(
  ...
  try {
    const payload = {
      ...
      theme_id: settings.theme_id || 'classic-card',
